...
$CheckMDT = ((Get-ItemProperty HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*).DisplayName -Match "Microsoft Deployment Toolkit").Length -gt 0
if ($CheckMDT) {
...
$CheckADK = ((Get-ItemProperty "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows Kits\Installed Roots\").KitsRoot10 -Match "Windows kits").Length -gt 0
if ($CheckADK) {
